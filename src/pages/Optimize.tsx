...
const API_URL = "https://promptforge-api.promptforge-app.workers.dev";
    ...
    <div className="min-h-[calc(100dvh-72px)] bg-[#0A0A0F] pt-10 pb-20 px-4">
      <div className="max-w-[720px] mx-auto">
        ...
        <motion.div initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} transition={{duration:0.6,ease:[0.16,1,0.3,1]}} className="text-center mb-10">
          ...
          <h1 className="text-3xl md:text-4xl lg:text-5xl font-bold text-white mb-3 tracking-tight" style={{fontFamily:"Space Grotesk,system-ui,sans-serif"}}>Optimize Your Prompt</h1>
          <p className="text-[#B4B4C7] text-base md:text-lg">Select a model, paste your prompt, let AI make it perfect.</p>
        </motion.div>
        ...
        <motion.div initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} transition={{duration:0.6,delay:0.2,ease:[0.16,1,0.3,1]}} className="bg-[rgba(19,19,31,0.7)] backdrop-blur-xl border border-[rgba(168,85,247,0.12)] rounded-2xl mb-4">
          <textarea value={prompt} onChange={e=>setPrompt(e.target.value)} placeholder={`Enter your ${model.name} prompt...`} className="w-full h-40 md:h-48 bg-transparent text-white placeholder-[#6B7280] p-4 md:p-5 resize-none focus:outline-none text-[15px] leading-relaxed" style={{fontFamily:"JetBrains Mono,monospace"}} spellCheck={false}/>
        ...
        <motion.div initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} transition={{duration:0.6,delay:0.3,ease:[0.16,1,0.3,1]}} className="mb-6">
          <motion.button whileHover={{scale:1.02,boxShadow:"0 0 40px rgba(168,85,247,0.4)"}} whileTap={{scale:0.98}} onClick={optimize} disabled={loading||!prompt.trim()} className="w-full min-h-14 py-4 rounded-2xl font-semibold text-white text-lg transition-all disabled:opacity-40 disabled:cursor-not-allowed" style={{background:"linear-gradient(135deg,#A855F7 0%,#7C3AED 40%,#06B6D4 100%)"}}>
        ...
        <AnimatePresence>{done && <motion.div initial={{opacity:0,y:30}} animate={{opacity:1,y:0}} transition={{duration:0.5,ease:[0.16,1,0.3,1]}} className="bg-[rgba(19,19,31,0.7)] backdrop-blur-xl border border-[rgba(6,182,212,0.2)] rounded-2xl overflow-hidden">
          ...
          {result && <div className="px-4 md:px-6 py-3 border-t border-[rgba(6,182,212,0.1)] flex items-center justify-between"><span className="text-[#6B7280] text-xs">{result.length} chars</span><span className="text-[#10B981] text-xs">+{Math.max(0,result.length-prompt.length)} added</span></div>}
        </motion.div>}</AnimatePresence>

        {!done && <motion.p initial={{opacity:0}} animate={{opacity:1}} transition={{delay:0.5}} className="mt-10 text-center text-[#6B7280] text-xs md:text-sm">Tip: The more specific your prompt, the better the optimization.</motion.p>}
      </div>
    </div>
  );
}
